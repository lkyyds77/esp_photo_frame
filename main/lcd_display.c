...
esp_err_t lcd_display_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)SHARED_SPI_HOST, &io_config, &io_handle));
...
void lcd_draw_bitmap(uint16_t x0, uint16_t y0, uint16_t w, uint16_t h, uint16_t *color_data) {
    if (s_panel_handle) {
        esp_lcd_panel_draw_bitmap(s_panel_handle, x0, y0, x0 + w, y0 + h, color_data);
    }
}
